...
class PaymentRequisitionForm(models.Model):
    ...
    payment_requisition_form_line_ids = fields.One2many(
        "payment.requisition.form.lines",
        "payment_requisition_form_id",
        string="payment requisition form lines",
        copy=True,
    )
    ...
    bank_details = fields.Char(string="Bank Details", tracking=True)
    ...
    total_amount_approved = fields.Float(
        ...
        compute="_total_amount_approved",
        readonly=True,
        tracking=True,
    )
    ...
    num_word = fields.Char(
        string="Amount Approved In Words:", compute="_compute_amount_in_word"
    )
    ...
    md_approval_date = fields.Date(string="Date", readonly=True, tracking=True)
    ...
    rejection_log_ids = fields.One2many(
        comodel_name="payment.requisition.rejection.log",
    ...
    def _total_amount_outstanding(self):
        for rec in self:
            rec.total_amount_outstanding = (
                rec.total_amount_approved - rec.total_amount_paid
            )
    ...
    def _has_no_outstanding_amount(self):
        for rec in self:
            if rec.total_amount_outstanding <= 0.0:
                return True
        return False
    
    def _has_all_payments_posted(self):
        for rec in self:
            if not all(state == "posted" for state in rec.payment_ids.mapped("state")):
                return False
        return True

    def _confirm_post(self):
        for record in self:
            print("Record state before posting <<<<<>>>>>>>>>>", record.state)
            has_no_outstanding = record._has_no_outstanding_amount()
            has_all_payments_posted = record._has_all_payments_posted()
            print("has_no_outstanding", has_no_outstanding)
            print("has_all_payments_posted", has_all_payments_posted)
            if has_no_outstanding and has_all_payments_posted:
                print("Posting the record >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
                record.state = "post"
            else:
                record.state = "approve"
            print("Record state after posting <<<<<>>>>>>>>>>", record.state)
            return False
    ...
    def _compute_total_payments(self):
        self.payment_count = (
            ...
            .sudo()
            .search_count([("requisition_id", "=", self.id)])
        )
    ...
    def unlink(self):
        for rec in self:
            if rec.state != "draft":
                raise UserError(_("You cannot delete a record that is not in draft"))

    def is_md(self, partner_id=None):
        if not partner_id:
            return False
        user = self.env["res.users"].search([("partner_id", "=", int(partner_id))])
        if user:
    ...
    def button_submit(self):
        ...
        is_internal_auditor = requester.has_group("topline.group_internal_audit")
        if any(
            line.amount_requested == 0
            for line in self.payment_requisition_form_line_ids
        ):
            raise UserError(
                "The following lines have no valid requested amount\n{}".format(
                    ", ".join(
                        self.payment_requisition_form_line_ids.filtered(
                            lambda l: l.amount_requested == 0
                        ).mapped("name")
                    )
                )
            )
        ...
        colleague_partners = employees_in_department.mapped("user_id").mapped(
            "partner_id"
        )
    ...
    def register_payment(self):
        return {
            ...
            "context": {
                ...
                "default_amount": self.total_amount_approved,
                "default_total_amount_outstanding": self.total_amount_outstanding,
            },
            "target": "new",
        }
    ...
    def button_line_manager_approval(self):
        ...
        self.message_subscribe(partner_ids=partner_ids)
        subject = (
            "Payment Requisition '{}', for {} has been approved by supervisor".format(
                self.name, self.employee_id.name
            )
        )
    ...
    def button_audit_approval_notification(self):
        ...
        group_id = self.env.ref("topline.group_md")
        user_ids = []
        partner_ids = []
        for user in group_id.users:
            user_ids.append(user.id)
    ...
    def button_md_approval_notification(self):
        ...
        if not is_md and user.has_group("payment_requisition.approve_payment_for_md"):
            ...
            if self.total_amount_approved > allowed_limit:
                raise UserError(
                    "This amount is above your allowed limit. You are allowed to approve amounts up to %s!"
                    % str(allowed_limit)
                )
    ...
    def button_finance_approval(self):
        for record in self:
            if any(
                line.amount_approved == False
                for line in record.payment_requisition_form_line_ids
            ):
                raise UserError(_("No approved amount on some lines."))
        self.write({"state": "approve"})
        self.finance_approval_date = date.today()
        self.finance_approval = self._uid
        subject = (
            "Payment Requisition '{}', for {} has been approved by Finance".format(
                self.name, self.employee_id.name
            )
        )
        partner_ids = []
    ...
    def _compute_amount_in_word(self):
        for rec in self:
            rec.num_word = (
                str(rec.currency_id.amount_to_text(rec.total_amount_approved)) + " only"
            )
...
class PaymentRequisitionFormLines(models.Model):
    ...
    def _check_user_group(self):
        ...
        if (
            ...
            or self.user_has_groups("topline.group_hr_line_manager")
            or self.user_has_groups("topline.group_internal_audit")
        ):
            is_manager = True
    ...
    state = fields.Selection(related="payment_requisition_form_id.state", store=True)
    ...
    amount_approved = fields.Float(string="Amount Approved", copy=False, tracking=True)
    ...
    account_id = fields.Many2one(
        ...
        string="Account",
        states={"post": [("readonly", True)], "done": [("readonly", True)]},
        help="An Payment account is expected",
    )
