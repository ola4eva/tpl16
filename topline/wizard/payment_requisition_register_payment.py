...
class PaymentRequisitionRegisterPaymentWizard(models.TransientModel):

    _name = "payment.requisition.register.payment.wizard"
    _description = "Payment Requisition Payment Wizard"

    @api.model
    def _default_partner_id(self):
        context = dict(self._context or {})
        active_ids = context.get('active_ids', [])
        payment_sheet = self.env['payment.requisition.form'].browse(active_ids)
        return payment_sheet.payee_id.id or payment_sheet.employee_id.id and payment_sheet.employee_id.address_home_id.id
    ...
    def _default_payment_amount(self):
        ...
        active_ids = context.get('active_ids', [])
        payment_sheet = self.env['payment.requisition.form'].browse(active_ids)
        payment_sheet.total_amount_approved_due
    ...
    amount = fields.Monetary(string='Payment Amount', required=True, default=_default_payment_amount)
    ...
    def _compute_payment_difference(self):
        if self.amount:
            self.payment_difference = self.test_amount - self.amount
    
    @api.onchange('partner_id')
    def _onchange_partner_id(self):
        active_ids = self._context.get('active_ids', [])
        payment_sheet = self.env['payment.requisition.form'].browse(active_ids)
        if payment_sheet.employee_id.id and payment_sheet.employee_id.sudo().bank_account_id.id:
            self.partner_bank_account_id = payment_sheet.employee_id.sudo().bank_account_id.id
    ...
    def payment_post_payment(self):
        ...
        body = (_("A payment of %s %s with the reference <a href='/mail/view?%s'>%s</a> related to your payment requisition %s has been made.") % (payment.amount, payment.currency_id.symbol, url_encode({'model': 'account.payment', 'res_id': payment.id}), payment.name, payment_sheet.name))
        payment_sheet.message_post(body=body)
        
        # Reconcile the payment and the expense, i.e. lookup on the payable account move lines
        account_move_lines_to_reconcile = self.env['account.move.line']
        for line in payment.move_line_ids + payment_sheet.account_move_id.line_ids:
            if line.account_id.internal_type == 'payable' and not line.reconciled:
                account_move_lines_to_reconcile |= line
        account_move_lines_to_reconcile.reconcile()
        
        #update amount due
        payment_sheet.total_amount_approved_due = self.payment_difference
        
        if payment_sheet.total_amount_approved_due <= 0:
            payment_sheet.write({'state': 'paid'})
