...
class account_payment(models.Model):
    ...
    def _synchronize_from_moves(self, changed_fields):
        for payment in self:
            if payment.payment_for == 'multi_payment':
                return True
        print ("=======_synchronize_from_moves=======",changed_fields)
    ...
    def more_amount_payment_line(self,amount):
        ...
        lq_default_line_name = ''.join(x[1] for x in self._get_liquidity_aml_display_name_list())
        cq_default_line_name = ''.join(x[1] for x in self._get_counterpart_aml_display_name_list())
        new_lines  = []
        liq_dic = {
            'name': liquidity_line_name or lq_default_line_name,
            ...
            # Receivable / Payable.
        rec_dict={
            'name': self.payment_reference or cq_default_line_name,
            'date_maturity': self.date,
        ...
        if self.payment_for == 'multi_payment':
            if self.move_id:
                if self.move_id.line_ids:
                    print ("=====",self.move_id, self.move_id.line_ids)
